...
#ifdef __APPLE_CC__
/* The current implementation on Mac OS X uses the system's shared library
   for standard I/O.  As a result, whether or not the buffer is GNU's
   standard I/O is irrelevant.  In the _STDIO_USES_IOSTREAM configuration,
   only one buffer is created.  So for the sake of efficiency, we assert
   this setting.  */
#include <stdio.h>
extern struct _IO_FILE_plus *_IO_stdin, *_IO_stdout, *_IO_stderr;
#define _IO_stdin_ (*(_IO_stdin))
#define _IO_stdout_ (*(_IO_stdout))
#define _IO_stderr_ (*(_IO_stderr))
#define _STDIO_USES_IOSTREAM 1
extern unsigned int _io_mode_bits;
#define _IO_IS_STDFILE(fp) ((fp->_fileno == 1) || (fp->_fileno ==2) || (fp->_fileno == 0))
#define _IO_OSX_SYNC_STDIO	0x01
#define _IO_WHICH_OFILE(fp) ((fp->_fileno == 1) ? stdout : stderr )
#else /* !__APPLE_CC__ */
